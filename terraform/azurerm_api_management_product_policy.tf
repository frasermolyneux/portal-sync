...
<policies>
  <inbound>
      <base/>
      <choose>
          <when condition="@(System.Text.RegularExpressions.Regex.IsMatch(context.Request.Url.Path, @"/api/health$"))">
          ...
          <otherwise>
              <validate-jwt header-name="Authorization" failed-validation-httpcode="401" failed-validation-error-message="JWT validation was unsuccessful" require-expiration-time="true" require-scheme="Bearer" require-signed-tokens="true">
                  ...
                  <audiences>
                  <audience>${local.sync_api.application.primary_identifier_uri}</audience>
                  </audiences>
                  <issuers>
